...
void setup()
{
  ...
  float quatK;
  float quatReal;
  while(validReadings != false) {
    while(imu.dataAvailable() != true);
    quatI = imu.getQuatI();
    quatJ = imu.getQuatJ();
    quatK = imu.getQuatK();
    quatReal = imu.getQuatReal();
    yawPrev = atan2((quatI * quatJ + quatReal * quatK), ((quatReal * quatReal + quatI * quatI) - 0.5f));
    pitchPrev = -asin(2.0f * (quatI * quatK - quatReal * quatJ));
    rollPrev = atan2((quatReal * quatI + quatJ * quatK), ((quatReal * quatReal + quatK * quatK) - 0.5f));
    if (isnan(yawPrev) || isnan(pitchPrev) || isnan(rollPrev)) {
      //validReadings = false;
    } else {
      validReadings = true;
    }
...
void loop() {
  ...
  bool valid = true;
  float dt = waitTime;
  while((millis() - t0) < waitTime) valid = false;
  if (valid == true) {
    // print actual time
    dt = t1 - t0;
  }
  if (dYaw >= 0) Serial.print('+');
  Serial.print(dYaw, 3);
  Serial.print(F(","));
  if (dt >= 0) Serial.print('+');
  Serial.print(dt, 3);
  Serial.println();
}
